...
Scenario: EApp application Better Alternative Group Free test, state specific TX, SOH include six questions, both product coningent beneficiary test scenarios
	...
	And Input valid Sponsor firstName, lastName, DOB
	And Click Sponsor gender 
	And Select Eligibility, Contribution Mode, BranchOfService, DutyStatus, Rank
	...
	And Select Better Alternative Roy click and selct Options
	...
	And Click Review Application Button to signin
	Then Input Last Name and Last four SSN number
	And Click Review and Sign My Application button to confirm
	Then Click Review Application Ducuments Button to review all the Application
	And Input Sponsor Last Name
	And Input Sponsor last four SSN
