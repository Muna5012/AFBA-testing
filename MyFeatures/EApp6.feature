@eApp6
Feature: EApp Application Better Alternative FedTerm (LT12) With Child rider. LT12 generic NV. Both LT12s Contingent Beneficiary test feature
Scenario: EApp Application Better Alternative FedTerm (LT12) With Child rider. LT12 generic NV. Both LT12s Contingent Beneficiary test scenarios

	When Home Page to click Manage cases button 
	Then Search AFBA cases on Manage Cases page
	Then Click the AFBA casess
	And Click the New Enrollement button to create case
	Then Input Sponsor SSN numbers
	And Input Enrollment city
	Then Select Enrollment state
	
	And click Begin Enrollement button to create new sponsor
	Then Input Sponsor First Name, Last Name and DOB
	Then Select Sponsor Gender
	And Click Marreid button 
	And Input Spouse FirstName, LastName and DOB
	
	Then Click Include Children button to input child information
	And Input Child First Name, Last Name and DOB 
	And Clilk Add Child button for add more child
	Then Input second child First Name and Last Name and DOB
	
	Then Select option Eligibility 
	And Select option Branch of service
	Then Select Option Duty Status
	And Select option Rank  
	Then Select Contribution Mode option
	Then Input Source Code
	And Input Street one, Street two, City, ST and Zip code
	Then Input Email Address and repeat Email address for confimation
	Then Input daytime phone and second phone
	Then Click Show Coverage Options to select Benefitiary Plans
	Then Waive Better Alternative 
	Then Covered Application Member Benefit five thousands k
	And Select Fed Term option to select Sponsor and spouse Tobacco or NonTobacco
	Then Select Coverage type for Customize sponsor and spouse 
	And Select Coverage type Rider Units
	And Click Answer No to all Questions 
	And Click Next button to input Sponsor informations
	Then Select Sponser height and Weight options
	And Click Next button to input spouse information
	Then Input Spouse SSN
	And Select Spouse gender option
	And Select spouse height and weight options
	Then Click Next button to fill out beneficiaries option
	And Input Sponsor primary beneficiary FullName,SSN, Relationship and DOB
	Then Input Spouse Primary Beneficiary fullname, SSN, Relationship and DOB
	And Click Next button to confirm all the informations
	
	And Select Location for Signing in person option
	Then Click I agree to this coverage and ready for sign button to signin
	Then Click Review Application Ducuments to Continue Button to review all the application
	And Input Sponsor Last Name for confirm
	And Input Sponsor last four SSN numbers for confirm
	Then Select Paper Option for receive the documents
	Then Click Sign Button to Signin 
	And Click Sign Button for eSignature to this application
	
	
		
			
	
	
	